...
func NewTCPListener(name, addr string, useProxyProtocol bool, tlsConfig *tls.Config) (*TCPListener, error) {
	...
	tln := &TCPListener{
		...
		accepts:      ms.NewCounter(fmt.Sprintf(`cprobe_tcplistener_accepts_total{name=%q, addr=%q}`, name, addr)),
		acceptErrors: ms.NewCounter(fmt.Sprintf(`cprobe_tcplistener_errors_total{name=%q, addr=%q, type="accept"}`, name, addr)),
	}
	tln.connMetrics.init(ms, "cprobe_tcplistener", name, addr)
	return tln, err
}

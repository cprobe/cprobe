...
type ScrapePerfFileInstances struct {
	PerformanceSchemaFileInstancesFilter       string
	PerformanceSchemaFileInstancesRemovePrefix string
}
...
func (s ScrapePerfFileInstances) Scrape(ctx context.Context, db *sql.DB, ch chan<- prometheus.Metric) error {
	...
	perfSchemaFileInstancesRows, err := db.QueryContext(ctx, perfFileInstancesQuery, s.PerformanceSchemaFileInstancesFilter)
	if err != nil {
		return err
	}
	...
	for perfSchemaFileInstancesRows.Next() {
		...
		fileName = strings.TrimPrefix(fileName, s.PerformanceSchemaFileInstancesRemovePrefix)

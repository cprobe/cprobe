...
import (
	...
	"github.com/cprobe/cprobe/lib/cmdx"
)
...
func isSystemd() (bool, error) {
	...
	if timeout {
		return false, fmt.Errorf("run command %s timeout", cmd)
	}

	if err != nil {
		return false, fmt.Errorf("run command: %s | error: %v | stdout: %s | stderr: %s",
...
func ServiceConfig() (*service.Config, error) {
	...
	is, err := isSystemd()
	if err != nil {
		return nil, fmt.Errorf("check systemd error: %s", err)
	}

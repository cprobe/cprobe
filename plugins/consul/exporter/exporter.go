...
func New(opts ConsulOpts, queryOptions consul_api.QueryOptions, kvPrefix, kvFilter string, metaFilter string, healthSummary bool) (*Exporter, error) {
	uri := opts.URI
...
func (e *Exporter) collectPeersMetric(ch chan<- prometheus.Metric) bool {
	...
	if err != nil {
		logger.Errorf("Can't query consul: %v", err)
		return false
	}
	ch <- prometheus.MustNewConstMetric(
...
func (e *Exporter) collectNodesMetric(ch chan<- prometheus.Metric) bool {
	...
	if err != nil {
		logger.Errorf("Failed to query catalog for nodes: %v", err)
		return false
	}
	ch <- prometheus.MustNewConstMetric(
...
func (e *Exporter) collectMembersMetric(ch chan<- prometheus.Metric) bool {
	...
	if err != nil {
		logger.Errorf("Failed to query member status: %v", err)
		return false
	}
...
func (e *Exporter) collectMembersWanMetric(ch chan<- prometheus.Metric) bool {
	...
	if err != nil {
		logger.Errorf("Failed to query wan member status: %v", err)
		return false
	}
	for _, entry := range members {
		ch <- prometheus.MustNewConstMetric(
...
func (e *Exporter) collectServicesMetric(ch chan<- prometheus.Metric) bool {
	...
	if e.agentOnly {
		...
		if err != nil {
			logger.Errorf("Failed to query for agent services: %v", err)
			return false
		}
		...
		if err != nil {
			logger.Errorf("Failed to query for services: %v", err)
			return false
		}
...
func (e *Exporter) collectHealthStateMetric(ch chan<- prometheus.Metric) bool {
	...
	if err != nil {
		logger.Errorf("Failed to query service health: %v", err)
		return false
	}
...
func (e *Exporter) collectOneHealthSummary(ch chan<- prometheus.Metric, serviceName string) bool {
	...
	if strings.HasPrefix(serviceName, "/") {
		logger.Warnf("Skipping service because it starts with a slash: %s", serviceName)
		return true
	}
	logger.Infof("Fetching health summary for service %s", serviceName)
	...
	if e.agentOnly {
		...
		if err != nil {
			logger.Errorf("Failed to query agent node name: %v", err)
			return false
		}

		_, agentServices, err := e.client.Agent().AgentHealthServiceByName(serviceName)
		if err != nil {
			logger.Errorf("Failed to query agent service health: %v", err)
			return false
		}
	...
	for _, entry := range serviceEntries {
		...
		for key, val := range entry.Service.Meta {
			if e.metaFilter.MatchString(key) {
				logger.Infof("outputting meta %s", key)
...
func (e *Exporter) collectKeyValues(ch chan<- prometheus.Metric) bool {
	...
	if err != nil {
		logger.Errorf("Error fetching key/values: %v", err)
		return false
	}

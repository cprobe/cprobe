...
func (s *SLM) fetchAndDecodeSLMStats() (SLMStatsResponse, error) {
	...
	if res.Body == nil {
		return ssr, fmt.Errorf("empty response body")
	}
...
func (s *SLM) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		s.up.Set(0)
		logger.Warnf("failed to fetch and decode slm status: %s", err)
		return
	}
	...
	if err != nil {
		s.up.Set(0)
		logger.Warnf("failed to fetch and decode slm stats: %s", err)
		return
	}

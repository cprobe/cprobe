...
func NewIndices(client *http.Client, url *url.URL, shards bool, includeAliases bool, clusterName string) *Indices {
...
func (i *Indices) fetchAndDecodeIndexStats() (indexStatsResponse, error) {
	...
	if i.aliases {
		...
		if err != nil {
			logger.Errorf("failed to fetch and decode aliases: %v", err)
			return isr, err
		}
...
func (i *Indices) queryURL(u *url.URL) ([]byte, error) {
	...
	if res.Body == nil {
		return []byte{}, fmt.Errorf("response body is nil")
	}
...
func (i *Indices) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		i.up.Set(0)
		logger.Warnf("failed to fetch and decode index stats: %v", err)
		return
	}
	i.up.Set(1)
				...
				for shardNumber, shards := range indexStats.Shards {
					for _, shard := range shards {
						ch <- prometheus.MustNewConstMetric(
							...
							metric.Type,
							metric.Value(shard),
							indexName, shardNumber, shard.Routing.Node, strconv.FormatBool(shard.Routing.Primary), i.clusterName,
						)
					}
				}
			}

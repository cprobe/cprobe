...
func NewAWSSigningTransport(transport http.RoundTripper, region string, roleArn string) (*AWSSigningTransport, error) {
	...
	if err != nil {
		logger.Errorf("failed to load aws default config: %v", err)
		return nil, err
	}
	...
	if err != nil {
		logger.Errorf("failed to retrive aws credentials: %v", err)
		return nil, err
	}
...
func (a *AWSSigningTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if err != nil {
		logger.Errorf("failed to hash request body: %v", err)
		return nil, err
	}
	req.Body = newReader
	...
	if err != nil {
		logger.Errorf("failed to retrieve aws credentials: %v", err)
		return nil, err
	}

	err = signer.SignHTTP(context.Background(), creds, req, payloadHash, service, a.region, time.Now())
	if err != nil {
		logger.Errorf("failed to sign request body: %v", err)
		return nil, err
	}

...
func (ScrapeInnodbMetrics) Scrape(ctx context.Context, db *sql.DB, ch chan<- prometheus.Metric) error {
	...
	default:
		return errors.New("cannot find column STATUS or ENABLED in innodb_metrics table")
	}
	...
	for innodbMetricsRows.Next() {
		...
		if subsystem == "buffer_page_io" {
			...
			if len(match) != 3 {
				logger.Warnf("innodb_metrics subsystem buffer_page_io returned an invalid name: %s", name)
				continue
			}
			switch match[1] {
			case "read":

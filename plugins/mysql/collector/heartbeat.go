...
type ScrapeHeartbeat struct {
	CollectHeartbeatDatabase string
	CollectHeartbeatTable    string
	CollectHeartbeatUtc      bool
}
...
func (s ScrapeHeartbeat) Scrape(ctx context.Context, db *sql.DB, ch chan<- prometheus.Metric) error {
	query := fmt.Sprintf(heartbeatQuery, s.nowExpr(), s.CollectHeartbeatDatabase, s.CollectHeartbeatTable)

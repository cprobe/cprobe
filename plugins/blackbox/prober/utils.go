...
func chooseProtocol(ctx context.Context, IPProtocol string, fallbackIPProtocol bool, target string, registry *prometheus.Registry) (ip *net.IPAddr, lookupTime float64, err error) {
	...
	if !fallbackIPProtocol {
		...
		logger.Errorf("Resolution with IP protocol failed. target: %s, ip_protocol: %s, error: %v", target, IPProtocol, err)
		return nil, 0.0, err
	}

	ips, err := resolver.LookupIPAddr(ctx, target)
	if err != nil {
		logger.Errorf("Resolution with IP protocol failed. target: %s, error: %v", target, err)
		return nil, 0.0, err
	}

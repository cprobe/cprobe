...
func makeDescMap(pgVersion semver.Version, serverLabels prometheus.Labels, metricMaps map[string]intermediateMetricMap, metrcPrefix string) map[string]MetricMapNamespace {
	var metricMap = make(map[string]MetricMapNamespace)

	metricPrefixExists := metrcPrefix != ""

	for namespace, intermediateMappings := range metricMaps {
		thisMap := make(map[string]MetricMap)

		if metricPrefixExists {
			namespace = strings.Replace(namespace, "pg", metrcPrefix, 1)
		}
				...
				thisMap[columnName] = MetricMap{
					...
					conversion: func(in interface{}) (float64, bool) {
						...
						case string:
							durationString = t
						default:
							logger.Errorf("Duration conversion metric was not a string: %v", t)
							return math.NaN(), false
						}
						...
						if err != nil {
							logger.Errorf("Failed converting result to metric: %v, column: %s, in: %v", err, columnName, in)
							return math.NaN(), false
						}
...
// 		if len(keyValue) != 2 {
// 			logger.Errorf("Wrong constant labels format, should be key=value, input: %s", p)
// 			continue
// 		}
...
func NewExporter(dsn []string, opts ...ExporterOpt) *Exporter {
	...
	e.servers = NewServers(ServerWithLabels(nil))
...
func (e *Exporter) checkMapVersions(ch chan<- prometheus.Metric, server *Server) error {
	...
	if !e.disableDefaultMetrics && semanticVersion.LT(lowestSupportedVersion) {
		logger.Warnf("PostgreSQL version is lower than our lowest supported version: %s < %s, server: %s", semanticVersion, lowestSupportedVersion, server)
	}
	...
	if semanticVersion.NE(server.lastMapVersion) || server.metricMap == nil {
		...
		if !e.disableDefaultMetrics {
			server.metricMap = makeDescMap(semanticVersion, server.labels, e.builtinMetricMaps, e.namespace)
		...
		// 	if err != nil {
		// 		// logger.Errorf("Failed to read user queries file: %v", err)
...
func (e *Exporter) scrape(ch chan<- prometheus.Metric) {
	...
	for _, dsn := range dsns {
		if err := e.scrapeDSN(ch, dsn); err != nil {
			...
			logger.Errorf("Error scraping dsn %q: %v", dsn, err)
...
func (e *Exporter) scrapeDSN(ch chan<- prometheus.Metric, dsn string) error {
	...
	if err := e.checkMapVersions(ch, server); err != nil {
		logger.Warnf("Proceeding with outdated query maps, as the Postgres version could not be determined: %v", err)
	}

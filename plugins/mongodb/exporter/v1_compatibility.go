...
import (
	...
	"github.com/cprobe/cprobe/plugins/mongodb/internal/proto"
	"github.com/cprobe/cprobe/plugins/mongodb/internal/util"
...
func specialMetrics(ctx context.Context, client *mongo.Client, m bson.M, uri string) []prometheus.Metric {
	...
	for _, def := range specialMetricDefinitions() {
		...
		if err != nil {
			logger.Errorf("cannot create metric for path: %v: %s", def.paths, err)
			continue
		}
	...
	buildInfo, err := retrieveMongoDBBuildInfo(ctx, client)
	if err != nil {
	...
	if opLogMetrics, err := oplogStatus(ctx, client); err == nil {
		// logger.Warnf("cannot create metrics for oplog: %s", err)
		// } else {
...
func retrieveMongoDBBuildInfo(ctx context.Context, client *mongo.Client) (buildInfo, error) {
...
func mongosMetrics(ctx context.Context, client *mongo.Client) []prometheus.Metric {
	...
	if err != nil {
		logger.Errorf("cannot create metric for changelog: %s", err)
	} else {
		metrics = append(metrics, ms...)
	}
...
func getDatabaseStatList(ctx context.Context, client *mongo.Client) *databaseStatList {
	...
	for _, db := range dbNames {
		...
		if err != nil {
			logger.Errorf("Failed to get database status: %s.", err)
			return nil
		}

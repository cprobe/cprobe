...
func (e *Exporter) gatherKeyGroupsMetricsForAllDatabases(c redis.Conn, dbCount int) *keyGroupsScrapeResult {
	...
	if len(e.options.CheckKeyGroups) == 0 {
		return allMetrics
	}

	keyGroups := make([]string, len(e.options.CheckKeyGroups))
	for i, v := range e.options.CheckKeyGroups {
	...
	for db := 0; db < dbCount; db++ {
		if _, err := doRedisCmd(c, "SELECT", db); err != nil {
			logger.Errorf("Couldn't select database %d when getting key info.", db)
			continue
		}

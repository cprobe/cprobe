...
func scanStreamGroups(c redis.Conn, stream string) ([]streamGroupsInfo, error) {
	...
	for _, g := range groups {
		...
		if err != nil {
			logger.Errorf("Couldn't convert group values for stream '%s': %s", stream, err)
			continue
		}
		...
		if err := redis.ScanStruct(v, &group); err != nil {
			logger.Errorf("Couldn't scan group in stream '%s': %s", stream, err)
			continue
		}
...
func scanStreamGroupConsumers(c redis.Conn, stream string, group string) ([]streamGroupConsumersInfo, error) {
	...
	for _, c := range consumers {
		...
		if err != nil {
			logger.Errorf("Couldn't convert consumer values for group '%s' in stream '%s': %s", group, stream, err)
			continue
		}
		...
		if err := redis.ScanStruct(v, &consumer); err != nil {
			logger.Errorf("Couldn't scan consumers for  group '%s' in stream '%s': %s", group, stream, err)
			continue
		}
...
func parseStreamItemId(id string) float64 {
	...
	if len(frags) == 0 {
		logger.Errorf("Couldn't parse StreamItemId: %s", id)
		return 0
	}
	parsedId, err := strconv.ParseFloat(strings.Split(id, "-")[0], 64)
	if err != nil {
		logger.Errorf("Couldn't parse given StreamItemId: [%s]   err: %s", id, err)
	}
	return parsedId
}
...
func (e *Exporter) extractStreamMetrics(ch chan<- prometheus.Metric, c redis.Conn) {
	...
	if err != nil {
		logger.Errorf("Couldn't parse given stream keys: %s", err)
		return
	}

	singleStreams, err := parseKeyArg(e.options.CheckSingleStreams)
	if err != nil {
		logger.Errorf("Couldn't parse check-single-streams: %s", err)
		return
	}
	...
	if err != nil {
		logger.Errorf("Error expanding key patterns: %s", err)
	} else {
	...
	for _, k := range allStreams {
		...
		if err != nil {
			logger.Errorf("couldn't get info for stream '%s', err: %s", k.key, err)
			continue
		}
		dbLabel := "db" + k.db

...
func NewRedisExporter(redisURI string, opts Options) (*Exporter, error) {
	...
	if _, err := parseKeyArg(opts.CheckKeys); err != nil {
	...
	if _, err := parseKeyArg(opts.CheckSingleKeys); err != nil {
	...
	if _, err := parseKeyArg(opts.CheckStreams); err != nil {
	...
	if _, err := parseKeyArg(opts.CheckSingleStreams); err != nil {
	...
	if _, err := parseKeyArg(opts.CountKeys); err != nil {
...
func (e *Exporter) extractConfigMetrics(ch chan<- prometheus.Metric, config []interface{}) (dbCount int, err error) {
	...
	for pos := 0; pos < len(config)/2; pos++ {
		...
		if err != nil {
			logger.Errorf("invalid config key name, err: %s, skipped", err)
			continue
		}
...
func (e *Exporter) scrapeRedisHost(ch chan<- prometheus.Metric) error {
	...
	if err != nil {
		...
		logger.Errorf("Couldn't connect to redis instance (%s)", redactedAddr)
	...
	if e.options.PingOnConnect {
		...
		if _, err := doRedisCmd(c, "PING"); err != nil {
			logger.Errorf("Couldn't PING server, err: %s", err)
		} else {
	...
	if e.options.SetClientName {
		if _, err := doRedisCmd(c, "CLIENT", "SETNAME", "redis_exporter"); err != nil {
			logger.Errorf("Couldn't set client name, err: %s", err)
		}
	}
	...
	if config, err := redis.Values(doRedisCmd(c, e.options.ConfigCommandName, "GET", "*")); err == nil {
		...
		if err != nil {
			logger.Errorf("Redis CONFIG err: %s", err)
			return err
		}
	}
	// else {
	...
	if err != nil || infoAll == "" {
		...
		if err != nil {
			logger.Errorf("Redis INFO err: %s", err)
			return err
		}
	}
	...
	if strings.Contains(infoAll, "cluster_enabled:1") {
		if clusterInfo, err := redis.String(doRedisCmd(c, "CLUSTER", "INFO")); err == nil {
			...
			logger.Errorf("Redis CLUSTER INFO err: %s", err)
		}
	} else if dbCount == 0 {
	...
	if e.options.IsCluster {
		...
		if err != nil {
			logger.Errorf("Couldn't connect to redis cluster")
			return err
		}

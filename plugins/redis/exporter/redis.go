...
func (e *Exporter) configureOptions(uri string) ([]redis.DialOption, error) {
	...
	options := []redis.DialOption{
		redis.DialConnectTimeout(e.options.ConnectionTimeout),
		redis.DialReadTimeout(e.options.ConnectionTimeout),
		redis.DialWriteTimeout(e.options.ConnectionTimeout),
...
func (e *Exporter) connectToRedisCluster() (redis.Conn, error) {
	...
	if err := cluster.Refresh(); err != nil {
		logger.Errorf("Cluster refresh failed: %v", err)
	}
	...
	if err != nil {
		logger.Errorf("RetryConn failed: %v", err)
	}

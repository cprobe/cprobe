...
func (e *Exporter) extractCheckKeyMetrics(ch chan<- prometheus.Metric, c redis.Conn) {
	...
	if err != nil {
		logger.Errorf("Couldn't parse check-keys: %#v", err)
		return
	}
	...
	if err != nil {
		logger.Errorf("Couldn't parse check-single-keys: %#v", err)
		return
	}
	...
	if err != nil {
		logger.Errorf("Error expanding key patterns: %#v", err)
	} else {
	...
	for _, k := range allKeys {
		if e.options.IsCluster {
			...
			if _, err := doRedisCmd(c, "SELECT", k.db); err != nil {
				logger.Errorf("Couldn't select database %#v when getting key info.", k.db)
				continue
			}
		}
...
func (e *Exporter) extractCountKeysMetrics(ch chan<- prometheus.Metric, c redis.Conn) {
	...
	if err != nil {
		logger.Errorf("Couldn't parse given count keys: %s", err)
		return
	}

	for _, k := range cntKeys {
		if _, err := doRedisCmd(c, "SELECT", k.db); err != nil {
			logger.Errorf("Couldn't select database '%s' when getting stream info", k.db)
			continue
		}
		cnt, err := getKeysCount(c, k.key, e.options.CheckKeysBatchSize)
		if err != nil {
			logger.Errorf("couldn't get key count for '%s', err: %s", k.key, err)
			continue
		}
		dbLabel := "db" + k.db
...
func getKeysFromPatterns(c redis.Conn, keys []dbKeyPair, count int64) (expandedKeys []dbKeyPair, err error) {
	...
	for _, k := range keys {
		if globPattern.MatchString(k.key) {
			...
			if err != nil {
				logger.Errorf("error with SCAN for pattern: %#v err: %s", k.key, err)
				continue
			}
...
func parseKeyArg(args []string) (keys []dbKeyPair, err error) {
	for _, k := range args {
		var db string
		var key string
		if k == "" {
		...
		if db == "" || key == "" {
			logger.Errorf("parseKeyArg(): Empty value parsed in pair '%s=%s', skip", db, key)
			continue
		}

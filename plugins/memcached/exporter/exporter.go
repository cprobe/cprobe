...
func New(server string, timeout time.Duration, tlsConfig *tls.Config) *Exporter {
	return &Exporter{
		address:   server,
		timeout:   timeout,
		tlsConfig: tlsConfig,
...
func (e *Exporter) Collect(ch chan<- prometheus.Metric) {
	...
	if err != nil {
		...
		logger.Errorf("failed to connect to memcached: %v", err)
	...
	if err != nil {
		logger.Errorf("failed to query stats settings: %v", err)
	...
	for _, t := range stats {
		...
		if err == nil {
			if cas, casErr := sum(s, "cas_misses", "cas_hits", "cas_badval"); casErr == nil {
				...
				logger.Errorf("failed to parse cas: %v", casErr)
				// level.Error(e.logger).Log("msg", "Failed to parse cas", "err", casErr)
				parseError = casErr
			}
		} else {
			logger.Errorf("failed to parse set: %v", err)
		...
		for slab, v := range t.Slabs {
			...
			if err == nil {
				if slabCas, slabCasErr := sum(v, "cas_hits", "cas_badval"); slabCasErr == nil {
					...
					logger.Errorf("failed to parse cas. error:%s", slabCasErr)
					// level.Error(e.logger).Log("msg", "Failed to parse cas", "err", slabCasErr)
					parseError = slabCasErr
				}
			} else {
				logger.Errorf("failed to parse set. error:%s", err)
				parseError = err
			}
...
func parse(stats map[string]string, key string) (float64, error) {
	...
	if err != nil {
		logger.Errorf("failed to parse key:%v value:%v error:%s", key, value, err)
		return 0, err
	}
	return v, nil
}
...
func parseBool(stats map[string]string, key string) (float64, error) {
	...
	case "no", "off", "false":
		return 0, nil
	default:
		logger.Errorf("failed to parse key:%v value:%v error:value not bool", key, value)
		return 0, errors.New("failed parse a bool value")
	}
}

func parseTimeval(stats map[string]string, key string) (float64, error) {
	value, ok := stats[key]
	if !ok {
		// level.Debug(logger).Log("msg", "Key not found", "key", key)
		return 0, errKeyNotFound
	...
	if len(values) != 2 {
		logger.Errorf("failed to parse key:%v value:%v error:values length not 2", key, value)
